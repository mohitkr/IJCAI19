...
Let $\mathbb{X}$ represents the set of all tensors \stefano{@Mohit: maybe it's best to differentiate between tensors, written \TX, and sets of tensors, say, $\mathbb{X}$?} $\TP(\mathbb{X})$ represents the power set of $\mathbb{X}$ and $\TE,\TF \subseteq \TP(\mathbb{X})$.
...
\label{eq:struct}
	...
	-\sum\limits_{\textbf{f} \in \TF}(\sum_{S_f}\prod_{\TY \in \textbf{f}} \TY_{M_{f,y}}) \le \TZ_{M}\\
	\forall \bigcup_{l \in E \cup F}(\bigcup_{\TY \in \textbf{l}} M_{l,y} / S_l) \bigcup M
\end{multline}
